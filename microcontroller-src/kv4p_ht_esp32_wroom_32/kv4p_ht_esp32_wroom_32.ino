...
const uint8_t COMMAND_SET_BLE_SERIAL = 7;   // Enable BLE Serial and save to EEPROM
...
{
public:
  DummyDRA818(HardwareSerial *serial, uint8_t type) {}
  int group(uint8_t bw, float freq_tx, float freq_rx, uint8_t ctcss_tx, uint8_t squelch, uint8_t ctcss_rx);
...
{
  Serial.printf("DRA818 handshake\n");
  return 1;
}
...
void loop()
{
  try
  {
    if (mode == MODE_STOPPED)
    {
      ...
      { // Read the delimiter and the command byte only (no params yet)
      ...
      case COMMAND_SET_BLE_SERIAL:
      {
        ...
        if (paramBytesMissing > 0)
        {
          ...
          bool bleStatus = (paramsStr.charAt(0) == '1');
    ...
    else if (mode == MODE_TX)
    {
      ...
      if (bytesAvailable > 0)
      {
        ...
        if (isTxCacheSatisfied)
        { // Note that it may have JUST been satisfied above, in which case we processed the cache, and will now process tempBuffer.
        ...
        for (int i = 0; i < bytesRead && i < TX_TEMP_AUDIO_BUFFER_SIZE; i++)
        {
          ...
          {
            if (tempBuffer[i] == COMMAND_DELIMITER[matchedDelimiterTokens])
...
void sendCmdToAndroid(byte cmdByte, const byte *params, size_t paramsLen)
{
  ...
  SerialDevice->write(outBytes, totalSize);
  SerialDevice->flush();
}
...
bool getBleSerial()
{
  enableBleSerial = EEPROM.read(0);

  return bool(enableBleSerial);
}
